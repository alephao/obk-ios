...
final class OpportunitiesViewController: BaseViewController, View {
    ...
    // MARK: - Initializing
    init(reactor: OpportunitiesViewReactor) {
    ...
    // MARK: - Configuring
    func bind(reactor: OpportunitiesViewReactor) {
        ...
        reactor.state.map { $0.isRefreshing }
            ...
            .subscribe(onNext: { [weak self] a in
                print("isRefreshing")
                print(a)
                a ? self?.refreshControl.beginRefreshing() : self?.refreshControl.endRefreshing()
            })
